...
func TestBitgetSpotFetcherFetchLastTradesWithRealData(t *testing.T) {
	...
	if 7 == len(trades) {
		...
		assert.Equal(t, int64(1735776414605), trades[0].ExecutedTimestamp)
		...
		assert.Equal(t, int64(1735761608327), trades[1].ExecutedTimestamp)
		...
		assert.Equal(t, int64(1735756412126), trades[2].ExecutedTimestamp)
		...
		assert.Equal(t, int64(1735754408738), trades[3].ExecutedTimestamp)
		...
		assert.Equal(t, int64(1735746364661), trades[4].ExecutedTimestamp)
		...
		assert.Equal(t, int64(1735569211032), trades[5].ExecutedTimestamp)
		...
		assert.Equal(t, int64(1735234336030), trades[6].ExecutedTimestamp)
	}
}
